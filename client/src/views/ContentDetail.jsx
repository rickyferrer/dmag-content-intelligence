...
function StatRow({ label, value, accent, title }) {
  return (
    <div title={title} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--border-subtle)', cursor: title ? 'help' : 'default' }}>
      <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{label}{title ? '*' : ''}</span>
      ...
      {item && (
        <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          {item.lifetimeValueBreakdown && (() => {
            ...
            return (
              <div>
                <h3 style={{ fontSize: 12, fontWeight: 500, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4 }}>Content Value Breakdown</h3>
                ...
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, marginBottom: 12 }}>
                  <div style={{ fontSize: 33, fontFamily: 'var(--font-mono)', fontWeight: 600, color: 'var(--accent-gold)', lineHeight: 1 }}>
                    {bd.score}
                  </div>
                  ...
                  {DIMS.map(({ key, label, info }) => {
                    ...
                    return (
                      <div key={key} title={info}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginBottom: 3, cursor: info ? 'help' : 'default' }}>
              ...
              <div style={{ background: 'var(--bg-elevated)', borderRadius: 6, padding: '4px 0' }}>
                {item.voices.map(v => (
                  <div key={v.voice} style={{ padding: '8px 14px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', gap: 10 }}>
                    <span style={{ fontSize: 12, color: 'var(--text-secondary)', flex: 1, textTransform: 'capitalize' }}>{v.voice}</span>
                    <div style={{ width: 60, height: 4, background: 'var(--border)', borderRadius: 2 }}>
                      <div style={{ height: '100%', borderRadius: 2, background: 'var(--accent-gold)', opacity: 0.7, width: `${(v.confidence || 0) * 100}%` }} />
                    </div>
                    <span style={{ fontSize: 11, color: 'var(--text-muted)', minWidth: 32, textAlign: 'right' }}>{((v.confidence || 0) * 100).toFixed(0)}%</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {item.newsletterHistory?.length > 0 && (() => {
            ...
            return (
              <div>
                <h3 style={{ fontSize: 12, fontWeight: 500, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4 }}>
                ...
                <ResponsiveContainer width="100%" height={140}>
                  <BarChart data={item.newsletterHistory} margin={{ left: 0, right: 10 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
                    <XAxis dataKey="week_start" tickFormatter={v => v?.slice(5, 10)} tick={{ fill: 'var(--text-muted)', fontSize: 10 }} stroke="var(--border)" />
                    <YAxis tick={{ fill: 'var(--text-muted)', fontSize: 10 }} stroke="var(--border)" allowDecimals={false} />
                    <Tooltip
                      contentStyle={{ background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 6, fontSize: 11 }}
                      labelStyle={{ color: 'var(--text-secondary)' }}
                    />
