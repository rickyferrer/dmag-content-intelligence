      ...
      {summary && (
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
            <span style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Articles</span>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 14, color: 'var(--text-primary)' }}>{summary.article_count.toLocaleString()}</span>
            <ChangeBadge change={summary.changes?.article_count} />
          </div>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
            <span style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Total Content Value</span>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 14, color: 'var(--accent-gold)' }}>{Math.round(summary.total_lifetime_value).toLocaleString()}</span>
            <ChangeBadge change={summary.changes?.total_lifetime_value} />
          </div>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
            <span style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Avg Content Value</span>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 14, color: 'var(--text-primary)' }}>{summary.avg_lifetime_value.toFixed(1)}</span>
