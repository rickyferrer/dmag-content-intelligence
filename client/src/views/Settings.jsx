            ...
            {Object.entries(syncStatus).map(([key, val]) => (
              <div key={key} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, padding: '6px 0', borderBottom: '1px solid var(--border-subtle)' }}>
                <span style={{ color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)' }}>
                  {key}
                  {val?.stale && (
                    <span style={{ marginLeft: 8, color: '#e8a33d', fontSize: 10 }} title={val.staleHours == null ? 'Never completed' : `No update in ${val.staleHours}h`}>
                      ⚠ {val.staleHours == null ? 'never run' : `stale (${val.staleHours}h)`}
                    </span>
                  )}
                </span>
                <span style={{ color: val?.stale ? '#e8a33d' : 'var(--text-muted)', fontSize: 11 }}>
              ...
              return (
                <div key={row.id} style={{
                  ...
                  <div style={{ minWidth: 160 }}>
                    <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-primary)' }}>{meta.label || row.benchmark_key}</div>
                    <span style={{
                      fontSize: 10, fontWeight: 600, color: badgeColor, background: badgeColor + '18',
                  ...
                  <div style={{ fontSize: 13, fontFamily: 'var(--font-mono)', marginLeft: 'auto', whiteSpace: 'nowrap' }}>
