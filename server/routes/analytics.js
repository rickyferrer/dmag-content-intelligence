...
// snapshot" to "the snapshot from N days ago" gives a real trend without
// restricting which articles count.
//
...
// Same shape as fetchSourceRows' output, but for an arbitrary [dateFrom,
// dateTo] range, from the site-wide daily table (source_daily). Pageviews per
// source are exact for the range and cover ALL traffic (not just the top
// articles the per-article snapshot holds). Users / loyal / in-market /
// newsletter only exist per article as rolling-30-day figures, so they're
// estimated: each source's latest-snapshot figure per pageview, times the
// range's real pageviews. Not scoped by publish date. Can't be filtered by
// content type (the daily series is site-wide) — the route falls back to the
// snapshot when a type is chosen.
function fetchSourceRowsDaily(db, { dateFrom, dateTo, snapshotRows }) {
  const days = db.prepare(`
    SELECT source, SUM(pageviews) AS total_pageviews
    FROM source_daily
    WHERE date >= ? AND date <= ?
    GROUP BY source
    HAVING total_pageviews > 0
    ORDER BY total_pageviews DESC
  `).all(dateFrom || '0000-01-01', dateTo || '9999-12-31');

  const snap = new Map(snapshotRows.map(r => [r.source, r]));
  return days.map(d => {
    const s = snap.get(d.source);
    const k = s && s.total_pageviews > 0 ? d.total_pageviews / s.total_pageviews : 0;
    return {
      source: d.source,
      total_pageviews: d.total_pageviews,
      article_count: s?.article_count || 0,
      total_users: Math.round((s?.total_users || 0) * k),
      total_loyal_users: Math.round((s?.total_loyal_users || 0) * k),
      total_inmarket: Math.round((s?.total_inmarket || 0) * k),
      total_newsletter_signups: Math.round((s?.total_newsletter_signups || 0) * k),
    };
  });
}
...
// signups) — used to diff a channel's current (latest) snapshot against an
// `asOf`-shifted one. Deliberately excludes the true GA4 channel-level
// metrics (Subscribe Clicks, Rev/1K, Clicks/1K, Score) sourced from
// source_performance, since THAT table is always its own trailing-30-day
// snapshot regardless of what we ask for, so comparing it against a shifted
// snapshot would compare two overlapping or nonsensical windows — see
// GA4_UNAVAILABLE_NOTES/note above for the same reasoning applied to the
// Efficiency columns.
...
router.get('/channels', (req, res) => {
  ...
  // Prefer true per-day data (source_daily) so the range actually
  ...
  const dailyCoverage = db.prepare('SELECT MIN(date) AS from_date, MAX(date) AS to_date, COUNT(*) AS n FROM source_daily').get();
  const range_mode = (dailyCoverage.n > 0 && !type) ? 'daily' : 'snapshot';
  const snapshot_reason = range_mode === 'daily' ? null : (type ? 'type_filter' : 'no_daily_data');

  const today = new Date().toISOString().slice(0, 10);
  const currentAsOf = (range_mode === 'snapshot' && dateTo && dateTo < today) ? dateTo + 'T23:59:59' : null;

  // The latest per-article snapshot is also the source of the per-pageview
  // ratios the daily estimates scale from.
  const latestSnapshotRows = fetchSourceRows(db, { type, asOf: currentAsOf });
  const sourceRows = range_mode === 'daily'
    ? fetchSourceRowsDaily(db, { dateFrom, dateTo, snapshotRows: latestSnapshotRows })
    : latestSnapshotRows;
